...
public class Course extends Canvas implements KeyListener, MouseListener, Runnable {
    ...
    private int seenPipes = -1;
    
    private int gui = 1; //1 = Main menu, 2 = Info, 3 = Leaderboards, 4 = Settings
    //private ArrayList<String> leaderboardNames; //not used
    //private ArrayList<Integer> leaderboardScores; //not used
    ...
    public Course(GameOptions gameOptions) 
    {
        try 
        {
            ...
            sc.useDelimiter("\\Z"); 
            String a = sc.next(); 
            int oldscore = Integer.parseInt(a);	
            highscore = oldscore;
            sc.close();
            }
           ...
           // leaderboardScores.add(1);
            //didn't end up making leaderboard instead just had a high score
        ...
        try 
        {
            ...
            if (playing == true && gameover == false) 
            {
                ...
                for( int i = 0; i < pipes.size(); i++)
                {
                    ...
                    if(i > seenPipes && pipe.getyPos() == 0 && bird.getxPos() + bird.getWidth()  > pipe.getxPos() + pipe.getWidth())
                    {      
                        score = score + 1;
                        seenPipes = seenPipes +2;
                    }
            ...
            if (playing && gameover) 
            {
                ...
                if (score > highscore) 
                {
                    ...
                    String a = String.valueOf(highscore);
